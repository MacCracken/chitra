...
| 004 | [JPEG decode pipeline](004-jpeg-decode-pipeline.md) | The JPEG path is `jpeg_huffman → jpeg_idct → jpeg_markers → jpeg`; the two frame-independent leaves are included *before* the frame-builder; `ChitraJpegFrame` handoff; MCU/plane/box-upsample model; the bit-reader's marker/restart handling; Cyrius logical-`>>` → signed `_jpeg_descale` | The baseline JFIF decoder (SOF0, gray + YCbCr, subsampling, DRI/RSTn) |
| 005 | [`alloc_reset()` breaks the next PNG decode](005-alloc-reset-sankoch-hazard.md) | sankoch memoizes its CRC-32 table as a raw pointer **into the bump arena** and cannot see a reset, so chitra's per-decode `crc32_init_table()` writes 16 KB through a dangling pointer | Any consumer reclaiming memory at the arena boundary — which is chitra's only reclamation story. PNG only; BMP and JPEG are unaffected |
